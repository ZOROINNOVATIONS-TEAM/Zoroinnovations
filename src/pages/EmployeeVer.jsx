...
import employeeData from "../Data/employeeData"; // Separate file for employee records
...
export default function EmployeeVer() {
  ...
  const handleVerify = () => {
    const found = employeeData.find((emp) => emp.id === searchId);
    setSelectedEmployee(found || null);
  };
  function handleEdit(emp) {
  alert(`Editing ${emp.name} (ID: ${emp.id})`);
  }
  function handleView(emp){
    alert(`the employee you are looking is 
      name:${emp.name} and id ${emp.id}`)
  }

  return (
   <div className=" bg-gradient-to-br from-[#0047ab] via-[#5d6bd9] to-[#ff7e5f]  p-6">
